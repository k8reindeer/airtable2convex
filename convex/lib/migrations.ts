...
export function genericMigration<TableName extends TableNames, MigrationCtx>({
                                                          batchSize,
                                                          migrateDoc,
                                                        }: {
  migrateDoc: (ctx: MutationCtx, doc: Doc<TableName>, migrationCtx: MigrationCtx) => Promise<any>;
  batchSize?: number;
}) {
  return mutation(
    async (
      ...
      for (const doc of page) {
        try {
          await migrateDoc(ctx, doc, migrationCtx);
        } catch (error) {
